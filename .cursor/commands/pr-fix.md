# /pr-fix — Create PR (fix) to main

You are an agent operating inside a git repository. Create a bugfix PR against `main` from current working tree changes.
...
2) Write a short summary (1–3 bullets) describing the bug fix.

## Branch name (always fix)
1) Create a concise slug from the summary: use lowercase, dash-separated.
2) Branch format: `fix/<slug>`.
...
- If currently on another branch:
  - If it already matches `fix/*`, keep it and reuse its name as `<branchName>`.
...
2) Create commit message:
   ...
   - Body: 2–6 bullets with key corrections (max ~80 chars per line)
3) Commit:
   - `git commit -m "<title>" -m "<body>"`
4) Verify the commit actually happened:
   - `git rev-parse --short HEAD`
   - `git log --oneline -1`
   - `git status --porcelain`
5) If HEAD did not change, or if staged changes remain after commit, stop and explain that the commit did not complete successfully.
